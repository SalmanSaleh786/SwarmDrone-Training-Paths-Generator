...
image_dict = {}  # Dictionary to store images by their name
...
def image(pos, id, scaledRatio, file="../../blueghost.gif"):
    ...
    if not os.path.exists(file):
        raise IOError("Image file not found: {}".format(file))
    x, y = pos
    img = None
    if id in image_dict:
        img = image_dict[id]
    ...
    return image_id  # Return the image ID if needed
    # _canvas.image = img  # Store reference
    # _canvas.image_list.append(img)  # Add the image to a list (global or class variable)
    #return _canvas.create_image(x, y, image=img)
...
def moveObject(id, pos, shape='circle', r=0, endpoints=None):
    global _canvas_x, _canvas_y

    x, y = pos

    if shape == 'circle':  # Handle the circle logic
        x0, x1 = x - r - 1, x + r
        y0, y1 = y - r - 1, y + r
        if endpoints == None:
            e = [0, 359]
    ...
    elif shape == 'rectangle':  # Handle rectangle logic
        width = r * 2  # Assuming `r` is half the width and height
        height = r * 2
        move_to(id, x - width // 2, y - height // 2)

    elif shape == 'image':  # Handle image logic
        move_to(id, x, y)

    # Add more shapes logic (if needed)
    # elif shape == 'line':
    #     pass
